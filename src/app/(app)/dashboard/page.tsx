...
export default async function DashboardPage() {
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        {canAddContract && (
          <Link href="/contracts/new"
            className="bg-brand text-white px-4 py-2.5 rounded-xl text-sm font-semibold hover:bg-brand-dark transition-colors flex items-center gap-2">
      ...
      <div className={`grid gap-4 mb-6 ${org?.plan !== 'free' ? 'grid-cols-2 md:grid-cols-3 lg:grid-cols-6' : 'grid-cols-2 md:grid-cols-3 lg:grid-cols-5'}`}>
        {[
          { label: 'Total contracts', value: stats.total, bg: 'bg-surface', border: 'border-border', text: 'text-navy' },
          { label: 'Active', value: stats.active, bg: 'bg-active-bg', border: 'border-border', text: 'text-active-text' },
          { label: 'Expiring soon', value: stats.expiring, bg: 'bg-expiring-bg', border: 'border-border', text: 'text-expiring-text' },
          { label: 'Expired', value: stats.expired, bg: 'bg-expired-bg', border: 'border-border', text: 'text-expired-text' },
          { label: 'Renewing this month', value: stats.thisMonth, bg: 'bg-brand-light', border: 'border-border', text: 'text-brand' },
          ...
          <div key={card.label} className={`${card.bg} ${card.border} border rounded-xl p-4`}>
            <div className={`${card.text} ${'small' in card ? 'text-lg' : 'text-2xl'} font-bold`}>{card.value}</div>
            ...
            {upcoming.map(c => {
              ...
              return (
                <Link key={c.id} href={`/contracts/${c.id}`}
                  className="flex items-center gap-4 px-5 py-4 hover:bg-page transition-colors group">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="font-medium text-navy text-sm truncate group-hover:text-brand">{c.vendor_name}</span>
                      <span className="text-xs bg-gray-100 text-slate-600 px-2 py-0.5 rounded-full flex-shrink-0">{contractTypeLabel(c.contract_type)}</span>
                      {c.auto_renews && <span className="text-xs bg-expiring-bg text-expiring-text px-2 py-0.5 rounded-full flex-shrink-0">Auto-renews</span>}
                    </div>
                    <div className="text-xs text-muted mt-0.5">Ends {formatDate(c.end_date)}</div>
                  </div>
                  <div className="text-sm text-slate-700 font-medium flex-shrink-0">
                    {c.value_annual ? formatCurrency(c.value_annual, c.currency) : '—'}
                  </div>
                  <div className={`text-xs font-semibold px-2.5 py-1 rounded-full flex-shrink-0 ${urgencyColor(days)}`}>
                    {urgencyLabel(days)}
                  </div>
                </Link>
              )
